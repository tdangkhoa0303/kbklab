...
const ReduxProvider: React.FC<PropsWithChildren<ReduxProviderProps>> = (props) => {
	...
	const composedEnhancer = useMemo(
		() => composeWithDevTools(applyMiddleware(thunkMiddleware, ...reduxConfigurations.middlewares)),
		[reduxConfigurations.middlewares]
	);

	const reduxStore = useMemo(() => createStore(rootReducer, composedEnhancer), [rootReducer, composedEnhancer]);
