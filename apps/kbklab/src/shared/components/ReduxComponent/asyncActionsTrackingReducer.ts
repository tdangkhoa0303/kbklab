...
const asyncActionsTrackingReducer = (state = initialAsyncActionsState, action: AnyAction) => {
	const {type} = action;
	...
	if (type.endsWith('/fulfilled') || type.endsWith('/rejected')) {
		return immutableState
			.set('numberOfRequests', (previousState: number) => previousState > 0 ? previousState - 1 : 0)
