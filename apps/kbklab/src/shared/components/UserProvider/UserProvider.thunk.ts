...
export const loginWithGoogle = createAsyncThunk<LoginResponse, LoginWithGooglePayload>(`${AppContext.User}/loginWithGoogle`, async (params) => {
	const response = await APIClient.post<LoginResponse>('users/auth/google', params);
	return response.data;
});
