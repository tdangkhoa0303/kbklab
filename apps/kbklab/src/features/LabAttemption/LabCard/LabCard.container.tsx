...
export type LabCardContainerPropsWithDefault = LabCardContainerProps & LabCardDefaultProps;
...
const LabCardContainer: React.FC<LabCardContainerProps> = (props) => {
	...
	const onEditClassLab: MouseEventHandler<HTMLButtonElement> = useCallback((event) => {
		event.stopPropagation();
		handleEditClassLab && handleEditClassLab(classLab)
	}, [classLab, handleEditClassLab]);

	const onDeleteClassLab: MouseEventHandler<HTMLButtonElement> = useCallback((event) => {
		event.stopPropagation();
