...
const ClassSelect: React.FC<ClassSelectProps> = ({classCode}) => {
	...
	const initialClassOption = useMemo(() => classOptions.find(option => option.value === classCode) || null, [classOptions, classCode]);
	const [currentClassOption, setCurrentClassOption] = useState<Option | null>(initialClassOption)

	const handleSelectClass = useCallback((_, option) => {
