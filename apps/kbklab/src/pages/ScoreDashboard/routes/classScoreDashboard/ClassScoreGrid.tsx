...
const ClassScoreGrid: React.FC<ClassScoreGridProps> = ({currentClass, ...props}) => {
	const columnDefs = useClassScoreGridColDefs(currentClass);
	const [studentScores, setStudentScores] = useState<StudentScore[]>([])

	useEffect(() => {
		APIClient
			.get(`scores/classScores?class=${currentClass.code}`)
			.then(response => setStudentScores(response.data.data))
	}, [currentClass]);
