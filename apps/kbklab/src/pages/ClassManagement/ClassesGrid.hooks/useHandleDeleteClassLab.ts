...
export const useDeleteClassLabStatus = (): ResponseStatus | null => useSelector(deleteClassLabStatusSelector);

export type HandleDeleteClassLab = (classLabId: string) => void;

export const useHandleDeleteClassLab = (currentClass: Class | undefined): HandleDeleteClassLab => {
	...
	return useCallback((classLabId: string) => {
		if(!currentClass) {
			return;
		}
