...
const PrivateOutlet: React.FC<PropsWithChildren<PrivateOutletProps>> = ({restrictedRole, minimumRole}) => {
	...
	const isAuthorized = useMemo((): boolean => {
		...
		const isInRestrictedList = !restrictedRole || restrictedRole.includes(userInfo.role);
	...
	if (!isAuthorized) {
		return <Navigate to={fallbackPageByUserRole[userInfo.role]} state={{from: location}} />;
	}
