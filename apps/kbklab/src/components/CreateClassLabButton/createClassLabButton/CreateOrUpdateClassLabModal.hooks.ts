...
export const useCreateClassLabInitialValues = (editingClassLab: ClassLab | undefined): CreateClassLabValues => {
	const editingValues = useMemo((): Partial<CreateClassLabValues> => {
		if(!editingClassLab) {
			return {}
		}

		const {id, startDate, endDate, class: classId, lab: {id: labId}} = editingClassLab;
		return {
			id,
			startDate,
			endDate,
...
export const useCreateOrUpdateClassLabOptions = (): CreateOrUpdateClassLabOptions => {
