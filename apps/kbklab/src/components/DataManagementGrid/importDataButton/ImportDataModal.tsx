...
export interface ImportDataModalProps<TImportFormValues extends FormikValues> extends Partial<FormikProps<TImportFormValues>> {
	onSubmit: (formValues: ImportDataFormValuesWithFile<TImportFormValues>) => void
...
const ImportDataModal = <TImportFormValues extends FormikValues, >(props: PropsWithChildren<ImportDataModalProps<TImportFormValues>>): React.ReactElement<PropsWithChildren<ImportDataModalProps<TImportFormValues>>> => {
	const {onSubmit, modalRef, sampleFileUrl, children, initialValues, FormikProps} = props;
	...
	const primaryButtonProps = useMemo((): Partial<ButtonProps> => ({
		disabled: isError,
