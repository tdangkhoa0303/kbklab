...
const ConfirmationModal: React.FC<PropsWithChildren<ConfirmationModalProps>> = ({children}) => {
	const [state, setState] = useState<ConfirmationModalState>(confirmationModalInitialState);
	const modalRef = useRef<ModalRef>(null);

	const {title, confirmText, cancelText, content, onConfirm} = state;

	const showConfirmation = useCallback((overriddenState: Partial<ConfirmationModalState>) => {
