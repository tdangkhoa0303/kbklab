...
const CreateOrUpdateClassLabModal: React.FC<
  CreateOrUpdateClassLabModalProps
> = (props) => {
  const {
    classOptions,
    labOptions,
  ...
  const handleSubmitForm = useCallback(
    ({ startDate, endDate, ...values }: CreateClassLabValues) => {
      onSubmit({
        ...values,
  ...
  return (
    ...
    >
      <FormikTextField hidden name="id" sx={{ display: 'none' }} />
      <Stack spacing={3}>
        <FormikTextField
          select
          fullWidth
