...
const omitDisabledFieldsFromPayload = (
  ...
  disabledFields: Dictionary<boolean>
): Partial<CreateClassLabValues> =>
  reduce(
    values,
    (result: Partial<CreateClassLabValues>, value, key) => {
...
export const useUpdateClassLabStatus = (): ResponseStatus | null =>
  useSelector(updateClassLabStatusSelector);
...
export const useUpdateClassLabHandler = (
  params: UseUpdateClassLabHandlerParams
): UpdateClassLabHandler => {
  ...
  return useCallback(
    (values: CreateClassLabValues) => {
      const valuesToSubmit = omitDisabledFieldsFromPayload(
        values,
        disabledFields
      );
      showToast();
