...
export const currentDetailClassSelector = createSelector(
  ...
  ({ entities }, currentDetailClassId): Class | undefined =>
    currentDetailClassId ? entities[currentDetailClassId] : undefined
);

export const updateClassLabStatusSelector = createSelector(
  classesUIStateSelector,
  (uiState: ClassesUIState): ResponseStatus | null =>
    uiState.updateClassLabStatus
);
...
export const classByCodeSelector = createSelector(
  ...
  allClassesSelector,
  (code, allClasses): Class | null =>
    allClasses.find((currentClass) => currentClass.code === code) || null
);
